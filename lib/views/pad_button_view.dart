...
import 'package:flutter_joystick/models/gestures.dart';
import 'package:flutter_joystick/models/pad_button_item.dart';
...
class PadButtonsView extends StatelessWidget {
  ...
  PadButtonsView({
    this.size,
    this.buttons = const [
      PadButtonItem(index: 0, child: Text('A')),
      PadButtonItem(index: 1, child: Text('B'), pressedColor: Colors.red),
      PadButtonItem(index: 2, child: Text('C'), pressedColor: Colors.green),
      PadButtonItem(index: 3, child: Text('D'), pressedColor: Colors.yellow),
    ],
    ...
    return Positioned(
      child: StatefulBuilder(builder: (context, setState) {
        return GestureDetector(
          ...
          onTapUp: (details) {
            _processGesture(paddButton, Gestures.tap_up);
          ...
          onTapDown: (details) {
            _processGesture(paddButton, Gestures.tap_down);

            setState(() =>
            buttonsStateMap[paddButton.index] = paddButton.pressedColor);
          },
          onTapCancel: () {
            _processGesture(paddButton, Gestures.tap_cancel);

            setState(() =>
            buttonsStateMap[paddButton.index] = paddButton.backgroundColor);
          },
          onLongPress: () {
            _processGesture(paddButton, Gestures.long_press);
          },
          onLongPressStart: (details) {
            _processGesture(paddButton, Gestures.long_press_start);

            setState(() =>
            buttonsStateMap[paddButton.index] = paddButton.pressedColor);
          },
          onLongPressUp: () {
            _processGesture(paddButton, Gestures.long_press_up);
